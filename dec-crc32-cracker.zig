...
var lastIndex: [256]u8 = undefined;
...
pub export fn getIndexes(_crc: u32) u32 {
    ...
    while (j < 4) : (j += 1) {
        ...
        indexes[j] = _crc_index;
        crc ^= table[_crc_index] >> (j << 3);
    }
    return @bitCast(indexes);
}
...
fn check(_i: u32) i32 {
    ...
    if (crc_index != indexes[3]) {
        return -1;
    }
    var low: u12 = 0;
    var i: u8 = 2;
    while (true) : (i -= if (i == 0) break else 1) {
        const num: u8 = @as(u8, @truncate(crc)) ^ indexes[i];
        if (num < '0' or num > '9') {
            return -1;
        }
        low = (low << 4) | (num - '0');
